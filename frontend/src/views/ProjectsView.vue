<template>
  <div class="projects-view">
    ...
    <GroupingRunDialog v-model="showGroupingDialog" @start="onGroupingStart" />

    <ProjectImportDialog v-model="showImportDialog" @imported="onImported" />

    <ComparisonSideBySide v-model="showCompare" :project-ids="compareIds" />
    ...
    <el-row :gutter="16" class="stats-row">
      ...
      <el-col :span="8">
        <el-statistic
          title="Авто-проверенных"
          :value="store.stats.auto_checked"
        />
      </el-col>
    </el-row>
    ...
    <template v-if="store.viewMode === 'list'">
      ...
      >
        ...
        <el-table-column label="Статус" width="120" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.is_selected" type="warning" size="small"
              >В отборе</el-tag
            >
            <el-tag v-else-if="row.is_auto_checked" type="success" size="small"
              >Проверен</el-tag
            >
    ...
    <transition name="float-panel">
      <div
        v-if="store.viewMode === 'list' && checkedRows.length >= 1"
        class="float-actions"
      >
        ...
        <AddToGroupPopover
          ...
          @done="onAddedToGroup"
        />
        <el-button size="small" @click="addToSelection"
...
function groupTagStyle(row: { group_source: string | null }) {
  return {
    border:
      row.group_source === "auto" ? "1px dashed #909399" : "1px solid #909399",
    color: "#606266",
  };
}
...
function syncFiltersFromQuery() {
  ...
  if (q.priority_direction_id)
    updates.priority_direction_id = String(q.priority_direction_id);
...
function syncQueryFromFilters() {
  ...
  if (f.priority_direction_id)
    query.priority_direction_id = f.priority_direction_id;
